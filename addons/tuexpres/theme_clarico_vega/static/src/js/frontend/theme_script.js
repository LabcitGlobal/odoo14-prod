...
odoo.define('theme_clarico_vega.theme_script', function(require) {
    ...
    sAnimations.registry.themeEvent = sAnimations.Class.extend({
        ...
        start: function() {
            ...
            $(".ept_price_slider_block").find(".price_filter_head").addClass("te_fa-minus").removeClass("te_fa-plus");
        },
        _onslide: function(){
        ...
        onShowClearVariant: function() {
         ...
         _onClickToRemove : function(ev) {
            $(ev.currentTarget).parents('.nav-item').find('.no_found_msg').remove();
            $(ev.currentTarget).parents('.nav-item').find('.te_s_attr_val').closest("li").removeClass('d-none');
         },
        ...
        _stickyGallery: function (scroll) {
            ...
            if($('.product_details_sticky').length){
                if($('div#product_details a#add_to_cart').length){
                    ...
                    var footerPosition = $("main").height() - $("#footer").height();
                    var productDetails = $('#product_details').height();
                    if ( scroll > productDetails && scroll < footerPosition - 400 ) {
                        if ($(window).width() >= 768) {
