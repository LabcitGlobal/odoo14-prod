...
class EmiproThemeBaseExtended(WebsiteSaleWishlist):
    ...
    def _get_search_domain(self, search, category, attrib_values, search_in_description=True):
        ...
        """
        if category or (attrib_values and list(attrib_values)):
            domain = self._get_custom_search_domain(search, category, attrib_values,
                ...
                if search_synonyms:
                    ...
                    for srch_synm in search_synonyms:
                        if srch_synm in [srch.lower().strip() for srch in search.split(" ")]:
                            continue
                        subdomains = [
                            [('name', 'ilike', srch_synm)],
                            [('product_variant_ids.default_code', 'ilike', srch_synm)],
                            [('description', 'ilike', srch_synm)],
                            [('description_sale', 'ilike', srch_synm)],
                            [('barcode', 'ilike', srch_synm)]
                        ]
                        if count:
                            domains.insert(0, '|')
                        count = True
                        domains.extend(i for i in expression.OR(subdomains))
                    copy_domain = domain.copy()
                    copy_domain.reverse()
                    index = len(domain) - copy_domain.index('|')
                    domain.insert(index, '|')
                    domain.extend(i for i in domains)
        ...
        if cust_max_val and cust_min_val:
            ...
            if products:
                ...
                domain = [('id', '=', False)]
        if attrib_values and list(attrib_values):
            attrib = None
            ids = []
            for value in list(attrib_values):
                if value[0] == 0:
                    ids.append(value[1])
    ...
    def _get_custom_search_domain(self, search, category, attrib_values,
                                  search_in_description=True):
        # return domain without attribute values
