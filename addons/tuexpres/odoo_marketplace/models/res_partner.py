...
class ResPartner(models.Model):
    ...
    def action_active_recommendation_fun(self):
        self.ensure_one()
        action = self.env.ref('odoo_marketplace.action_seller_recommendation')
        list_view_id = self.env['ir.model.data'].xmlid_to_res_id(
            'odoo_marketplace.seller_recommendation_tree_view')
        kanban_view_id = self.env['ir.model.data'].xmlid_to_res_id(
            'odoo_marketplace.seller_recommendation_kanban_view')
        return {
            'name': action.name,
            'help': action.help,
            'type': action.type,
            'views': [[kanban_view_id, 'kanban'],[list_view_id, 'tree']],
            'binding_view_types': action.binding_view_types,
            'view_mode': action.view_mode,
            'target': action.target,
            'res_model': action.res_model,
            'domain': "[('seller_id','=',%s)]" % self._ids[0],
        }
