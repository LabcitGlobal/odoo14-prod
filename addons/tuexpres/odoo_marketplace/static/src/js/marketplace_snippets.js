    ...
    publicWidget.registry.wkMarketplaceSnippets = publicWidget.Widget.extend({
        ...
        _fetch: function () {
            return this._rpc({
                ...
                var btn_content = 'Abrir mi tienda';
                if (res == '/my/marketplace'){
                    btn_content = 'Ir al panel de mi tienda';
                }
                var store_btn_el = `<a href="${res}" class="btn" style="font-weight:600;background:#ef0078;border-radius: 2px;color:#fff;text-transform: uppercase;">${btn_content}</a>`;
                return store_btn_el;
            });
        },
    });
