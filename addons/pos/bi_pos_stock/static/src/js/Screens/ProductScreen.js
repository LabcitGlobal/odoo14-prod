...
odoo.define('bi_pos_stock.productScreen', function(require) {
	...
	class StockErrorPopup extends AbstractAwaitablePopup {        
        constructor() {            
            super(...arguments);
        }        
    }
    StockErrorPopup.template = "StockErrorPopup";

    Registries.Component.add(StockErrorPopup);
			...
			async _onClickPay() {
				...
				if(pos_config.pos_display_stock)
				{
					if (pos_config.show_stock_location == 'specific')
					{
						...
						if (pos_config.pos_online_check_stock)
						{
							let partner_id = self.env.pos.get_client();
							let location = self.env.pos.locations;
							let prods = [];
							...
							await this.rpc({
								model: 'stock.quant',
								method: 'get_products_stock_all_qty',
								args: [partner_id ? partner_id.id : 0, prods],
							}).then(function(output) {
								var flag = 0;
								let err_prods = [];
								for (var i = 0; i < lines.length; i++) {
											...
											var keys = $.map(output[0], function(value, key) {
												if (lines[i].product.type == 'product' && lines[i].product['id'] == key ){
													if (allow_order == false && lines[i].quantity > value){
														...
														if ( sw == true) {
															err_prods.push({
																'product' : lines[i].product.display_name,
								...
								if(flag > 0){
									call_super = false;  
									let { confirmed, payload } = self.showPopup("StockErrorPopup", {
										product: err_prods,                            
									});								
								}
							});
							...
							$.each(lines, function( i, line ){
								if (line.product.type == 'product'){
									if (allow_order == false && line.quantity > line.product['bi_on_hand']){
										call_super = false; 										
										let err_prods = 'PROD[' + line.product['display_name'] + '] - QTY[' + line.quantity + '] - STOCK[' + line.product['bi_on_hand'] + ']';
										self.showPopup('ErrorPopup', {
											title: self.env._t('Denied Order'),
											body: self.env._t(err_prods),
										});
										return
									}
									...
									if(allow_order == true && check < deny_order){
										call_super = false; 
										let err_prods = 'PROD[' + line.product['display_name'] + '] - QTY[' + line.quantity + '] - STOCK[' + line.product['bi_on_hand'] + '] - DENY[' + deny_order + ']';
										self.showPopup('ErrorPopup', {
											title: self.env._t('Denied Order'),
											body: self.env._t(err_prods),
										});
