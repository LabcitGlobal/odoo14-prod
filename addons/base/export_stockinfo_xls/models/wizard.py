...
class StockReport(models.TransientModel):
    ...
    def get_lines(self, data, warehouse):
        ...
        if categ_id:
            ...
            categ_products = self.env['product.product'].search([], order='categ_id asc, name asc')
        ...
        for obj in categ_products:
            if obj.with_context({'warehouse': warehouse}).qty_available > 0:
                sale_value = 0
                purchase_value = 0
    ...
    def get_xlsx_report(self, data, response):
        ...
        sheet.merge_range(1, 7, 2, 10, 'Información del Stock', format0)
        ...
        if d1:
            ...
            cat = cat.join(c)
            sheet.merge_range(4, 0, 4, 1, 'Categoría(s) : ', format4)
        ...
        sheet.merge_range('A8:G8', 'Fecha de Reporte: ' + str(times.strftime("%Y-%m-%d %H:%M %p")), format1)
        sheet.merge_range(7, 7, 7, count, 'Almacenes', format1)
        sheet.merge_range('A9:G9', 'Información del Producto', format11)
        w_col_no = 6
        w_col_no1 = 7
        ...
        sheet.write(9, 6, 'Costo', format21)
        p_col_no1 = 7
        for i in get_warehouse[0]:
            sheet.write(9, p_col_no1, 'Disponible', format21)
            sheet.write(9, p_col_no1 + 1, 'Virtual', format21)
            sheet.write(9, p_col_no1 + 2, 'Entrante', format21)
            sheet.write(9, p_col_no1 + 3, 'Despacho', format21)
            sheet.merge_range(9, p_col_no1 + 4, 9, p_col_no1 + 5, 'En Mano', format21)
            sheet.merge_range(9, p_col_no1 + 6, 9, p_col_no1 + 7, 'Total Vendido', format21)
            sheet.merge_range(9, p_col_no1 + 8, 9, p_col_no1 + 9, 'Total Comprado', format21)
            sheet.write(9, p_col_no1 + 10, 'Valuación', format21)
